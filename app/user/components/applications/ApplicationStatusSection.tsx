...
import { FiSearch, FiExternalLink, FiCheck, FiClock, FiEye, FiUsers, FiLoader } from 'react-icons/fi';
import JobListingCard from "../jobs/jobCard";
...
const ApplicationStatusSection: React.FC<ApplicationStatusSectionProps> = ({ 
  ...
  const jobApplications = applications && applications.length > 0
    ? applications.map((app: JobApplication) => ({
        id: app.id?.toString() || '0',
  ...
  const selectedJob = selectedApplicationId 
    ...
    : jobApplications.length > 0 ? jobApplications[0] : null;
            ...
            <div className="bg-white p-4 rounded-lg border border-gray-200">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">
                  {jobApplications.filter(app => ['Reviewing', 'Interviewed'].includes(app.status)).length}
                </p>
            ...
            <div className="bg-white p-4 rounded-lg border border-gray-200">
              <div className="text-center">
                <p className="text-2xl font-bold text-green-600">
                  {jobApplications.filter(app => ['Hired', 'Offered'].includes(app.status)).length}
                </p>
              ...
              <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {filteredApplications.map((job) => (
                    ...
                    >
                      <h3 className="font-semibold text-gray-800 hover:text-teal-600 text-sm">{job.title}</h3>
                      ...
                      <div className="flex justify-between items-center mt-2 text-xs">
                        <span className={`flex items-center gap-1 font-medium ${
                          ...
                          {job.status === 'Applied' || job.status === 'Hired' ? <FiCheck size={12} /> : <FiClock size={12} />}
                          {job.status}
                        </span>
